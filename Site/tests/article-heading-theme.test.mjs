...
test('article headings use the shared Era Spine hierarchy without new colours', () => {
  ...
  const articleScope = String.raw`\.sl-markdown-content:not\(:has\(\.home-gateway\)\)`;

  assert.ok(start >= 0 && end > start, 'expected a dedicated Era Spine hierarchy');
  assert.match(hierarchy, new RegExp(`${articleScope} h1\\s*\\{[\\s\\S]*font-size:\\s*clamp`));
  assert.match(hierarchy, new RegExp(`${articleScope} h2\\s*\\{[\\s\\S]*padding-inline-start:\\s*1rem[\\s\\S]*font-family:\\s*var\\(--vc-font-display\\)`));
  assert.match(hierarchy, new RegExp(`${articleScope} h2::before\\s*\\{[\\s\\S]*background:\\s*var\\(--era-heading-accent, var\\(--sl-color-accent-high\\)\\)[\\s\\S]*clip-path:\\s*polygon`));
  assert.match(hierarchy, new RegExp(`${articleScope} h3\\s*\\{[\\s\\S]*color:\\s*var\\(--era-heading-accent, var\\(--sl-color-accent-high\\)\\)[\\s\\S]*text-transform:\\s*uppercase`));
  assert.match(hierarchy, new RegExp(`${articleScope} h4\\s*\\{[\\s\\S]*font-family:\\s*var\\(--vc-font-body\\)[\\s\\S]*font-style:\\s*italic`));
  assert.match(hierarchy, new RegExp(`${articleScope} h5\\s*\\{[\\s\\S]*font-family:\\s*var\\(--vc-font-ui\\)[\\s\\S]*text-transform:\\s*uppercase`));
  assert.match(hierarchy, new RegExp(`${articleScope} h6\\s*\\{[\\s\\S]*color:\\s*var\\(--sl-color-gray-4\\)`));
